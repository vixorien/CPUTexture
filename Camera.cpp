...
Ray Camera::GetRayThroughPixel(float x, float y, int screenWidth, int screenHeight)
{
	// Create the ray and origin
	Ray ray = {};
	ray.Origin = transform.GetPosition();

    // Calculate NDCs
	XMFLOAT4 ndc(x, y, 0, 1);
	...
	XMMATRIX invVP = XMMatrixInverse(0, v * p);

	// Unproject the NDCs (including perspective divide)
	XMVECTOR unprojPos = XMVector4Transform(XMLoadFloat4(&ndc), invVP);
	unprojPos = XMVectorScale(unprojPos, 1.0f / XMVectorGetW(unprojPos));

	// Calculate direction through this position
	XMVECTOR dir = XMVectorSubtract(unprojPos, XMLoadFloat3(&ray.Origin));
	dir = XMVector3Normalize(dir);
	
	// Finalize ray direction
